...
import (
	...
	"github.com/vibtreeofficial/protocol/utils/guid"
)
...
func DispatchRulePriority(info *media_router.SIPDispatchRuleInfo) int32 {
	...
	default:
		return last
	case *media_router.SIPDispatchRule_DispatchRuleDirect:
		if rule.DispatchRuleDirect.GetPin() != "" {
			priority = 0
		} else {
			priority = 100
		}
	case *media_router.SIPDispatchRule_DispatchRuleIndividual:
		if rule.DispatchRuleIndividual.GetPin() != "" {
			priority = 1
		} else {
			priority = 101
		}
	case *media_router.SIPDispatchRule_DispatchRuleCallee:
...
func hasHigherPriority(r1, r2 *media_router.SIPDispatchRuleInfo) bool {
...
func SortDispatchRules(rules []*media_router.SIPDispatchRuleInfo) {
...
func ValidateDispatchRules(rules []*media_router.SIPDispatchRuleInfo, opts ...MatchDispatchRuleOpt) error {
...
func ValidateDispatchRulesIter(it iters.Iter[*media_router.SIPDispatchRuleInfo], opts ...MatchDispatchRuleOpt) (best *media_router.SIPDispatchRuleInfo, _ error) {
...
func NewDispatchRuleValidator(opts ...MatchDispatchRuleOpt) *DispatchRuleValidator {
	...
	return &DispatchRuleValidator{
		opt:       opt,
		byRuleKey: make(map[dispatchRuleKey]*media_router.SIPDispatchRuleInfo),
	}
}
...
type DispatchRuleValidator struct {
	...
	byRuleKey map[dispatchRuleKey]*media_router.SIPDispatchRuleInfo
}

func (v *DispatchRuleValidator) ValidateIter(it iters.Iter[*media_router.SIPDispatchRuleInfo]) iters.Iter[*media_router.SIPDispatchRuleInfo] {
	return &dispatchRuleValidatorIter{v: v, it: it}
}

func (v *DispatchRuleValidator) Validate(r *media_router.SIPDispatchRuleInfo) error {
...
type dispatchRuleValidatorIter struct {
	...
	it iters.Iter[*media_router.SIPDispatchRuleInfo]
}

func (v *dispatchRuleValidatorIter) Next() (*media_router.SIPDispatchRuleInfo, error) {
	r, err := v.it.Next()
	if err != nil {
		return nil, err
	}
...
func SelectDispatchRule(rules []*media_router.SIPDispatchRuleInfo, req *rpc.EvaluateSIPDispatchRulesRequest, opts ...MatchDispatchRuleOpt) (*media_router.SIPDispatchRuleInfo, error) {
...
func GetPinAndRoom(info *media_router.SIPDispatchRuleInfo) (room, pin string, err error) {
...
// This function delegates to media_router.NormalizeNumber for the actual implementation.
func NormalizeNumber(num string) string {
	return media_router.NormalizeNumber(num)
}

func validateTrunkInbound(byInbound map[string]*media_router.SIPInboundTrunkInfo, t *media_router.SIPInboundTrunkInfo, opt *matchTrunkOpts) error {
...
func ValidateTrunks(trunks []*media_router.SIPInboundTrunkInfo, opts ...MatchTrunkOpt) error {
	return ValidateTrunksIter(iters.Slice(trunks), opts...)
}

// ValidateTrunksIter checks a set of trunks for conflicts.
func ValidateTrunksIter(it iters.Iter[*media_router.SIPInboundTrunkInfo], opts ...MatchTrunkOpt) error {
	defer it.Close()
	...
	opt.defaults()
	byOutboundAndInbound := make(map[string]map[string]*media_router.SIPInboundTrunkInfo)
	for {
		t, err := it.Next()
		if err == io.EOF {
			break
		...
		if len(t.Numbers) == 0 {
			...
			if byInbound == nil {
				byInbound = make(map[string]*media_router.SIPInboundTrunkInfo)
...
type TrunkMatchResult struct {
	...
	Trunk *media_router.SIPInboundTrunkInfo
...
func MatchTrunk(trunks []*media_router.SIPInboundTrunkInfo, call *rpc.SIPCall, opts ...MatchTrunkOpt) (*media_router.SIPInboundTrunkInfo, error) {
...
func MatchTrunkDetailed(it iters.Iter[*media_router.SIPInboundTrunkInfo], call *rpc.SIPCall, opts ...MatchTrunkOpt) (*TrunkMatchResult, error) {
	defer it.Close()
	var opt matchTrunkOpts
	for _, fnc := range opts {
		fnc(&opt)
	...
	var (
		selectedTrunk    *media_router.SIPInboundTrunkInfo
		defaultTrunk     *media_router.SIPInboundTrunkInfo
		defaultTrunkPrev *media_router.SIPInboundTrunkInfo
		sawAnyTrunk      bool
	)
...
func (opt *matchTrunkOpts) defaults() {
	if opt.Filtered == nil {
		opt.Filtered = func(_ *media_router.SIPInboundTrunkInfo, _ TrunkFilteredReason) bool {
			return false
		}
	}
	if opt.Conflict == nil {
		opt.Conflict = func(_, _ *media_router.SIPInboundTrunkInfo, _ TrunkConflictReason) {}
	}
	if opt.Replace == nil {
		opt.Replace = func(t *media_router.SIPInboundTrunkInfo) *media_router.SIPInboundTrunkInfo {
			return t
		}
	}
}
...
type TrunkFilteredFunc func(tr *media_router.SIPInboundTrunkInfo, reason TrunkFilteredReason) bool
...
type TrunkConflictFunc func(t1, t2 *media_router.SIPInboundTrunkInfo, reason TrunkConflictReason)
...
type TrunkReplaceFunc func(t *media_router.SIPInboundTrunkInfo) *media_router.SIPInboundTrunkInfo
...
// Returns nil if no rules matched or an error if there are conflicting definitions.
func MatchTrunkIter(it iters.Iter[*media_router.SIPInboundTrunkInfo], call *rpc.SIPCall, opts ...MatchTrunkOpt) (*media_router.SIPInboundTrunkInfo, error) {
...
//
// Deprecated: use MatchDispatchRuleIter
func MatchDispatchRule(trunk *media_router.SIPInboundTrunkInfo, rules []*media_router.SIPDispatchRuleInfo, req *rpc.EvaluateSIPDispatchRulesRequest, opts ...MatchDispatchRuleOpt) (*media_router.SIPDispatchRuleInfo, error) {
...
func (opt *matchDispatchRuleOpts) defaults() {
	if opt.Conflict == nil {
		opt.Conflict = func(_, _ *media_router.SIPDispatchRuleInfo, _ DispatchRuleConflictReason) {}
	}
	if opt.Replace == nil {
		opt.Replace = func(r *media_router.SIPDispatchRuleInfo) *media_router.SIPDispatchRuleInfo {
			return r
		}
	}
}
...
type DispatchRuleConflictFunc func(r1, r2 *media_router.SIPDispatchRuleInfo, reason DispatchRuleConflictReason)
...
type DispatchRuleReplaceFunc func(r *media_router.SIPDispatchRuleInfo) *media_router.SIPDispatchRuleInfo
...
// Trunk parameter can be nil, in which case only wildcard dispatch rules will be effective (ones without Trunk IDs).
func MatchDispatchRuleIter(trunk *media_router.SIPInboundTrunkInfo, rules iters.Iter[*media_router.SIPDispatchRuleInfo], req *rpc.EvaluateSIPDispatchRulesRequest, opts ...MatchDispatchRuleOpt) (*media_router.SIPDispatchRuleInfo, error) {
	...
	var (
		specificRule    *media_router.SIPDispatchRuleInfo
		specificRuleCnt int
		defaultRule     *media_router.SIPDispatchRuleInfo
		defaultRuleCnt  int
	)
	noPin := req.NoPin
	sentPin := req.GetPin()
...
func EvaluateDispatchRule(projectID string, trunk *media_router.SIPInboundTrunkInfo, rule *media_router.SIPDispatchRuleInfo, req *rpc.EvaluateSIPDispatchRulesRequest) (*rpc.EvaluateSIPDispatchRulesResponse, error) {
	call := req.SIPCall()
	sentPin := req.GetPin()
	...
	enc := media_router.SIPMediaEncryption_SIP_MEDIA_ENCRYPT_DISABLE
	if trunk != nil {
	...
	attrs[media_router.AttrSIPCallID] = call.LkCallId
	...
	if rule.HidePhoneNumber {
		...
		attrs[media_router.AttrSIPPhoneNumber] = call.From.User
		attrs[media_router.AttrSIPHostName] = call.From.Host
		attrs[media_router.AttrSIPTrunkNumber] = call.To.User
	}
	...
	attrs[media_router.AttrSIPDispatchRuleID] = rule.SipDispatchRuleId
	...
	if krispEnabled {
		resp.EnabledFeatures = append(resp.EnabledFeatures, media_router.SIPFeature_KRISP_ENABLED)
	}
	return resp, nil
}
