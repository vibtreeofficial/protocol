...
import (
	...
	"github.com/vibtreeofficial/protocol/utils/must"
...
type TypedSignalClient = SignalClient[media_router.NodeID]
type TypedSignalServer = SignalServer[media_router.NodeID]

func NewTypedSignalClient(nodeID media_router.NodeID, bus psrpc.MessageBus, opts ...psrpc.ClientOption) (TypedSignalClient, error) {
	return NewSignalClient[media_router.NodeID](bus, psrpc.WithClientOptions(opts...), psrpc.WithClientID(string(nodeID)))
}

func NewTypedSignalServer(nodeID media_router.NodeID, svc SignalServerImpl, bus psrpc.MessageBus, opts ...psrpc.ServerOption) (TypedSignalServer, error) {
	return NewSignalServer[media_router.NodeID](svc, bus, psrpc.WithServerOptions(opts...), psrpc.WithServerID(string(nodeID)))
}

type TypedRoomManagerClient = RoomManagerClient[media_router.NodeID]
type TypedRoomManagerServer = RoomManagerServer[media_router.NodeID]

func NewTypedRoomManagerClient(bus psrpc.MessageBus, opts ...psrpc.ClientOption) (TypedRoomManagerClient, error) {
	return NewRoomManagerClient[media_router.NodeID](bus, opts...)
}
...
func NewTypedRoomManagerServer(svc RoomManagerServerImpl, bus psrpc.MessageBus, opts ...psrpc.ServerOption) (TypedRoomManagerServer, error) {
	return NewRoomManagerServer[media_router.NodeID](svc, bus, opts...)
}

type ParticipantTopic string
type RoomTopic string

func FormatParticipantTopic(roomName media_router.RoomName, identity media_router.ParticipantIdentity) ParticipantTopic {
	return ParticipantTopic(fmt.Sprintf("%s_%s", roomName, identity))
}

func FormatRoomTopic(roomName media_router.RoomName) RoomTopic {
...
func (t topicFormatter) ParticipantTopic(ctx context.Context, roomName media_router.RoomName, identity media_router.ParticipantIdentity) ParticipantTopic {
	return FormatParticipantTopic(roomName, identity)
}

func (t topicFormatter) RoomTopic(ctx context.Context, roomName media_router.RoomName) RoomTopic {
	return FormatRoomTopic(roomName)
}

type TopicFormatter interface {
	ParticipantTopic(ctx context.Context, roomName media_router.RoomName, identity media_router.ParticipantIdentity) ParticipantTopic
	RoomTopic(ctx context.Context, roomName media_router.RoomName) RoomTopic
}
...
type KeepalivePubSub interface {
	KeepaliveClient[media_router.NodeID]
	KeepaliveServer[media_router.NodeID]
}
...
func NewKeepalivePubSub(params ClientParams) (KeepalivePubSub, error) {
	client, err := NewKeepaliveClient[media_router.NodeID](params.Args())
	if err != nil {
		return nil, err
	}
	server := must.Get(NewKeepaliveServer[media_router.NodeID](nil, params.Bus))
