...
func guidPrefix[T media_router.Guid]() string {
	var id T
	switch any(id).(type) {
	case media_router.TrackID:
		return TrackPrefix
	case media_router.ParticipantID:
		return ParticipantPrefix
	case media_router.RoomID:
		return RoomPrefix
	default:
		panic("unreachable")
	}
...
func Marshal[T media_router.Guid](id T) media_router.GuidBlock {
	return media_router.GuidBlock(MarshalAppend(nil, id))
}

func MarshalAppend[T media_router.Guid](b []byte, id T) []byte {
	off := len(b)
...
func Unmarshal[T media_router.Guid](b media_router.GuidBlock) T {
	prefix := guidPrefix[T]()
