...
func Validate(info *media_router.IngressInfo) error {
	if info == nil {
...
func ValidateForSerialization(info *media_router.IngressInfo) error {
	if info == nil {
		return ErrInvalidIngress("missing IngressInfo")
	}

	if info.InputType != media_router.IngressInput_RTMP_INPUT && info.InputType != media_router.IngressInput_WHIP_INPUT && info.InputType != media_router.IngressInput_URL_INPUT {
		return ErrInvalidIngress("unsupported input type")
	}

	// Validate source
	switch info.InputType {
	case media_router.IngressInput_RTMP_INPUT,
		media_router.IngressInput_WHIP_INPUT:
		if info.StreamKey == "" {
			return ErrInvalidIngress("no stream key")
		}
	case media_router.IngressInput_URL_INPUT:
		if info.Url == "" {
...
func ValidateBypassTranscoding(info *media_router.IngressInfo) error {
	if !info.BypassTranscoding {
		return nil
	}

	if info.InputType != media_router.IngressInput_WHIP_INPUT {
...
func ValidateEnableTranscoding(info *media_router.IngressInfo) error {
...
func ValidateVideoOptionsConsistency(options *media_router.IngressVideoOptions) error {
	if options == nil {
		return nil
	}

	switch options.Source {
	case media_router.TrackSource_UNKNOWN,
		media_router.TrackSource_CAMERA,
		media_router.TrackSource_SCREEN_SHARE:
		// continue
	default:
	...
	case nil:
		// Default, continue
	case *media_router.IngressVideoOptions_Preset:
		_, ok := media_router.IngressVideoEncodingPreset_name[int32(o.Preset)]
		if !ok {
			return NewInvalidVideoParamsError("invalid preset")
		}

	case *media_router.IngressVideoOptions_Options:
...
func ValidateVideoEncodingOptionsConsistency(options *media_router.IngressVideoEncodingOptions) error {
	if options == nil {
	...
	case media_router.VideoCodec_DEFAULT_VC,
		media_router.VideoCodec_H264_BASELINE,
		media_router.VideoCodec_VP8:
		// continue
	default:
		return NewInvalidVideoParamsError("video codec unsupported")
	}

	layersByQuality := make(map[media_router.VideoQuality]*media_router.VideoLayer)
	...
	for q := media_router.VideoQuality_LOW; q <= media_router.VideoQuality_HIGH; q++ {
...
func ValidateAudioOptionsConsistency(options *media_router.IngressAudioOptions) error {
	if options == nil {
		return nil
	}
	...
	switch options.Source {
	case media_router.TrackSource_UNKNOWN,
		media_router.TrackSource_MICROPHONE,
		media_router.TrackSource_SCREEN_SHARE_AUDIO:
		// continue
	default:
	...
	case nil:
		// Default, continue
	case *media_router.IngressAudioOptions_Preset:
		_, ok := media_router.IngressAudioEncodingPreset_name[int32(o.Preset)]
		if !ok {
			return NewInvalidAudioParamsError("invalid preset")
		}

	case *media_router.IngressAudioOptions_Options:
...
func ValidateAudioEncodingOptionsConsistency(options *media_router.IngressAudioEncodingOptions) error {
	if options == nil {
		return NewInvalidAudioParamsError("empty options")
	}

	switch options.AudioCodec {
	case media_router.AudioCodec_DEFAULT_AC,
		media_router.AudioCodec_OPUS:
		// continue
	default:
