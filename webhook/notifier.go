...
type NotifyParams struct {
	ExtraWebhooks []*media_router.WebhookConfig
	Secret        string
}

type NotifyOption func(*NotifyParams)

func WithExtraWebhooks(wh []*media_router.WebhookConfig) NotifyOption {
...
type QueuedNotifier interface {
	RegisterProcessedHook(f func(ctx context.Context, whi *media_router.WebhookInfo))
	SetKeys(apiKey, apiSecret string)
	SetFilter(params FilterParams)
	QueueNotify(ctx context.Context, event *media_router.WebhookEvent, opts ...NotifyOption) error
	Stop(force bool)
}
...
func (n *DefaultNotifier) QueueNotify(ctx context.Context, event *media_router.WebhookEvent, opts ...NotifyOption) error {
	for _, u := range n.notifiers {
	...
	for _, wh := range p.ExtraWebhooks {
		lopts := []NotifyOption{
			WithExtraWebhooks([]*media_router.WebhookConfig{wh}),
		}
...
func (n *DefaultNotifier) RegisterProcessedHook(hook func(ctx context.Context, whi *media_router.WebhookInfo)) {
	for _, u := range n.notifiers {
...
func eventKey(event *media_router.WebhookEvent) string {
...
func logFields(event *media_router.WebhookEvent, url string) []interface{} {
...
func webhookInfo(
	event *media_router.WebhookEvent,
	queuedAt time.Time,
	...
	isDropped bool,
	sendError error,
) *media_router.WebhookInfo {
	whi := &media_router.WebhookInfo{
		EventId:         event.Id,
		Event:           event.Event,
