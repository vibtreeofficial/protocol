...
type AutoEncodedOutput interface {
	GetFileOutputs() []*media_router.EncodedFileOutput
	GetSegmentOutputs() []*media_router.SegmentedFileOutput
}

type EncodedOutput interface {
	AutoEncodedOutput
	GetStreamOutputs() []*media_router.StreamOutput
	GetImageOutputs() []*media_router.ImageOutput
}

type EncodedOutputDeprecated interface {
	GetFile() *media_router.EncodedFileOutput
	GetStream() *media_router.StreamOutput
	GetSegments() *media_router.SegmentedFileOutput
}

type DirectOutput interface {
	GetFile() *media_router.DirectFileOutput
	GetWebsocketUrl() string
}

type UploadRequest interface {
	GetS3() *media_router.S3Upload
	GetGcp() *media_router.GCPUpload
	GetAzure() *media_router.AzureBlobUpload
	GetAliOSS() *media_router.AliOSSUpload
}
